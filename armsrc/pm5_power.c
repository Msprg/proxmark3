...
//-----------------------------------------------------------------------------
// Proxmark5 (AT32F435) low-power idle.
//
// Idle heat on the PM5 is dominated by the 288 MHz PLL, the 1.3 V LDO and the
// main loop busy-polling. When the device has nothing to do, the main loop:
//   - takes SCLK off the PLL onto HICK-48, powers the PLL down and lowers the
//     LDO to 1.1 V (PWC_LDO_OUTPUT_1V1 allows up to 144 MHz);
//   - stops the FPGA 24 MHz clock (CLKOUT1 is PLL-sourced) with PA8 parked low;
//   - halts the core with WFI until an IRQ or a 1 ms SysTick wake.
// USB runs crystal-less off HICK-48 (ACC-trimmed against the SOF, see
// usb_cdc_at32.c), so it never sees the PLL go away. The 1 kHz tick counter is
// on the ERTC (HEXT/20) and is clock-independent too.
//
// Everything else keeps its 288 MHz assumptions: the boost is taken before a
// command / standalone mode runs and released afterwards. Ported from the
// Fantasi firmware's "reduce idle power" change.
//-----------------------------------------------------------------------------
#include "pm5_power.h"
...
void pm5_power_set_lowpower(bool on) {
    s_lowpower = on;
...
bool pm5_power_get_lowpower(void) {
    return s_lowpower;
}
...
void pm5_power_print_status(void) {
    ...
    Dbprintf("  Low-power idle...... %s", s_lowpower ? _GREEN_("enabled") : _YELLOW_("disabled"));
